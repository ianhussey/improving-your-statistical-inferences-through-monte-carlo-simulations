...
Results are cached to disk: the simulation is only run the first time, and read back in on subsequent renders. Delete the .rds file if you change the simulation code.

```{r}
dir.create("results", showWarnings = FALSE)

if(file.exists("results/simulation_template_hypothesis_testing.rds")){

  simulation <- read_rds("results/simulation_template_hypothesis_testing.rds")
  ...
  simulation <- experiment_parameters_grid |>
    ...
    mutate(results = future_pmap(.l = list(data = data),
                                 ...
                                 .progress = TRUE,
                                 .options = furrr_options(seed = TRUE))) |>
    # the generated data sets are not needed after they have been analysed, and keeping
    # them would make the saved file enormous
    select(-data)

  write_rds(x = simulation,
            file = "results/simulation_template_hypothesis_testing.rds",
            compress = "gz")
