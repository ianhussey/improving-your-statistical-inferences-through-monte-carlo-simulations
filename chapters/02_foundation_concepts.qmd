...
# very simple code (replicate(), hist()) rather than the cached-to-disk simulation
# workflow used later in the book, so caching is switched on here instead of
# complicating the code. knitr stores each chunk's results in
# 2_foundation_concepts_cache/ and re-uses them until that chunk's code changes.
execute:
  cache: true
---
...
<!-- TODO: add content on positive and negative controls -->
<!-- TODO: integrate some repetitive content below -->
<!-- TODO: integrate information from slides, or the slides themselves -->
<!-- TODO: remove old and new content redundancy -->
...
simulation_results <- experiment_parameters |>
  mutate(
    generated_data = pmap(
      .l = list(
        mean_number_of_attendees = mean_number_of_attendees,
        sd_number_of_attendees   = sd_number_of_attendees,
        mean_pizza_eaten         = mean_pizza_eaten,
        sd_pizza_eaten           = sd_pizza_eaten,
        mean_bottles_drank       = mean_bottles_drank,
        sd_bottles_drank         = sd_bottles_drank),
      .f = generate_data
      ),
...
5. Summarize over the iterations
6. Compare between the experiment conditions
