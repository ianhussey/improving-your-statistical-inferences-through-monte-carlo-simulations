...
### Help writing the 'do stuff': Pseudocode

Before you write the code inside a function, it’s often helpful to write pseudocode first.

Pseudocode is a plain-language (or semi-code) description of what your function should do, step by step. It is not valid R code, and it is not meant to run. Instead, it’s a thinking and planning tool: you use it to clarify the logic of your function before you worry about syntax.

Pseudocode has a number of advantages. Specifically:
...
- It separates the problem from the programming. You can get the logic right first, then translate it into R.
...
- It helps you communicate with others (and with your future self). A colleague might not understand your code immediately, but they can usually understand pseudocode.
...
Here is pseudocode for our sum_and_absolute() example:

```{text}

FUNCTION sum_and_absolute takes inputs x and y
  1. Add x and y to get their sum
  2. Take the absolute value of that sum
  3. Return the absolute value
END FUNCTION

```

You can also do this in a way that looks a little more like actual code:

```{text}

INPUT(S): x, y
temp_sum = x + y
result = absolute_value(temp_sum)
OUTPUT(S): result


```

Other ways of writing pseudocode are also fine, as long as the steps are unambiguous and it makes it clear exactly what the function is supposed to take as input, the steps it is intended to follow, and the intended output.

#### Going from pseudocode to R code

Once you have pseudocode, you can translate each step into R code. For example:
1. 'Add x and y' becomes `sum(x, y)` 
2. 'Take absolute value' becomes `abs()`
3. 'Return result' becomes `return(result)`

#### Pseudocode for a more typical simulation function

The examples we have explored so far have mainly been for the purposes of introducing you to writing functions. Although you have not yet got to the point where you're writing more complex simulation functions, it is still worth looking quickly at a pseudocode example of this. Indeed, the real value of pseudocode shows up when functions get longer than one line. For example, here’s pseudocode for a very common simulation function that we will explore in the next chapter: generate one dataset.

```{text}

FUNCTION generate_data takes inputs n_per_group, mu_control, mu_treatment, sd
  1. Create a vector of group labels: "control" and "treatment"
  2. For control group, draw n_per_group values from a normal distribution with mean mu_control and sd
  3. For treatment group, draw n_per_group values from a normal distribution with mean mu_treatment and sd
  4. Combine both groups into one dataset (a data frame)
  5. Return the dataset
END FUNCTION

```

Notice something important: this is basically just a checklist. When you convert it to R, you can translate each step one at a time and test as you go. This makes it much easier to debug your function, too!
...
```{r}
sum_and_absolute <- function(a, b){
  result <- abs(sum(x, y))
  return(result)
}

sum_and_absolute(1, 5)
```

::: {.callout-note collapse="true" title="Click to show answer"}
The function's inputs are defined as a and b, but it actually uses x and y. x and y were defined as global variables earlier in the script, so it will run using these two values every time, eg `sum_and_absolute(1, 5)` and `sum_and_absolute(31, 39)` will return the same result.
