...
simulation <- 
  ...
  # note the name on the second element: the grid column is `n_maximum` but the
  # argument of generate_data() is `n_max`, so the two have to be matched explicitly
  mutate(generated_data = pmap(.l = list(n_minimum = n_minimum,
                                         n_max = n_maximum,
  ...
  mutate(analysis_results = pmap(.l = list(data = generated_data,
