...
You have previously written a data-generation function (`generate_data()`) and an analysis function (`analyze()`), then called them once with the pipeline:
...
The answer: **mapping**. In the tidyverse, this is usually done with the `{purrr}` package (or `{furrr}` for parallel processing, which is basically a faster way to do this).

The goal of this lesson is to ultimately get you to understand how to use `pmap()`: a function that is fundamental to simulation workflows. To get you there, we're going to introduce some simpler functions first, which will help to give you an understand of how the whole `map()` family of functions work.
...
## **Why copy–paste is the enemy \[NOTE TO IAN BELOW\]**

\[NOTE TO IAN: THIS MAY NEED TO CHANGE AS IT REFERS TO A PREVIOUS CHAPTER'S CONTENT\]
...
map(
  .x = numbers, # .x is our list of inputs
  .f = ~ .x + 10 # .f is our function
  )
...
For example: maybe we want to vary all values of n_per_condition, mean_control, mean_intervention, and sd.

This brings us to the key function for this chapter: `pmap()`.

With `pmap()`, we supply all of the parameters as a list, using the .l argument. Our function (`generate_data()`) is supplied, like before, to the .f argument. We'll use the same approach as we used previously: run `pmap()` within a `mutate()` call, then `map()` the result onto `analyze()`, then call `unnest()` on the results.

Note the key difference: because `pmap()` takes parameter input as a list, this means we can in principle specify an unlimited number of parameters!
...
simulation_results <- experiment_parameters |>
  mutate(
    generated_data = pmap(
      .l = list(n_per_condition, mean_control, mean_intervention, sd), # our list of parameters
      .f = generate_data # our data-generating function
      ),
    results = map(generated_data, analyze)
  ) |>
  unnest(results)
...
Now that we have `pmap()`, we have all of the ingredients for running the main type of simulation workflow we will use in this course:
