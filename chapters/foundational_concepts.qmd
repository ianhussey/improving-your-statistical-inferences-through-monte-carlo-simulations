...
replicate(n = 100000, expr = t.test(x = rnorm(n = 50, m = 0, sd = 1), y = rnorm(n = 50, m = 0, sd = 1), var.equal = TRUE)$p.value) |> hist()
...
To very briefly show you these components in the simulation of *p*-values under the null hypothesis, we can build the code up in stages.
...
We can do this under two different conditions. (1) under the null hypothesis, where the population effect size is zero, and (2) under the alternative hypothesis where the population effect size is large (Cohen's d = 0.8). 
